...
class TelemetryBatchJsonTest {
  ...
  void testJsonBuilding() {
    ...
    long later = now + 1500;
    Collection<Metric> metrics = Collections
        .singletonList(new Count("mmm", 12.1, now, later, new Attributes().put("f1", "b1")));
    TelemetryBatch<Metric> batch = TelemetryBatch
        .batchMetrics(metrics, new Attributes().put("c1", "d1"));

    String commonBit = "{\"common\": \"wad\"}";
    String telemetryBit = "{\"bunchOf\": \"telemetry\"}";

    JsonCommonBlockWriter commonWriter = new JsonCommonBlockWriter() {
      @Override
    ...
    JsonTelemetryBlockWriter mainBodyWriter = new JsonTelemetryBlockWriter() {
      @Override
